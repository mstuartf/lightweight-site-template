...
const deleteBarber = (barberId: string): Promise<void> => {
  return getBarbers().then(existingBarbers => {
    set(
      ref(db, "/barbers"),
      existingBarbers.filter(({ id }) => id !== barberId)
    );
  });
};
    ...
    barbers.forEach(({ name, id }) => {
      const el = elFromStr(`
            <div id="${id}" class="flex justify-between items-center border rounded p-4 shadow">
                <div>${name}</div>
                <button type="button" class="focus:outline-none" id="${id}-btn">
      ...
      barbersContainer.appendChild(el);
      document
        .getElementById(`${id}-btn`)
        .addEventListener("click", () =>
          deleteBarber(id).then(() => getBarberList())
        );
    });
  });
};
